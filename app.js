...
database.ref('system_users').once('value').then((snapshot) => {
    if(snapshot.val()) {
    ...
    Object.entries(SYSTEM_USERS).forEach(([uId, user]) => {
        ...
        let actionsHtml = `
            <div style="display: flex; gap: 8px; margin-top: 10px;">
                ${auditarBtn}
                <button class="btn-primary" style="flex: ${flexEdit}; background: #FF9800; font-size:0.8rem; padding:10px;" onclick="abrirEditarEntrenador('${uId}')">✏️ Editar</button>
                <button class="btn-danger" style="flex: 1; padding:10px; font-size:0.9rem;" onclick="borrarEntrenador('${uId}', '${user.name}')">🗑️</button>
            </div>
        `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-ejecutar-ia').addEventListener('click', () => {
        ...
        guardarBaseDeDatos(); pintarDatosGuardados(); autogenModal.classList.add('hidden'); mostrarAlerta("🪄 IA Mágica", "Semana generada asegurando máxima variabilidad.", false);
    });
    ...
    document.querySelectorAll('.task-list').forEach(list => {
        new Sortable(list, { group: 'tareas-semanales', animation: 150, ghostClass: 'sortable-ghost', dragClass: 'sortable-drag',
            onEnd: function (evt) {
                ...
                appDB.fechas[toDateStr].tareas.splice(newIndex, 0, tareaMovida); guardarBaseDeDatos();
            }
        });
    });
}
